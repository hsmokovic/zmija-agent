    ...
    def mutate(self):
        ...
        for i in range(len(self.weights)):
            ...
            for j in range(weight.shape[0]):
                for k in range(weight.shape[1]):
                    # za svaku težinu generira se random broj iz uniformne razdiobe
                    weight_mut = np.random.uniform(0, 1)
                    # ako je random vr. manja od zadane vjerojatnosti mutation_prob -> mutacija
                    if weight_mut < neural['mutation_prob']:
        ...
        for i in range(len(self.bias)):
            ...
            for j in range(bias.shape[0]):
                # za svaki bias generira se random broj iz uniformne razdiobe
                bias_mut = np.random.uniform(0, 1)
                # ako je random vr. manja od zadane vjerojatnosti mutation_prob -> mutacija
                if bias_mut < neural['mutation_prob']:
    ...
    for i in range(len(parent1.weights)):
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                # ako je random vrijednost manja od 0.5
                # dijete preuzima težinu parenta1
                if np.random.uniform() < 0.5:
                    child_w[i, j] = parent1_w[i, j]
                # inače dobiva težinu parenta2
                else:
                    child_w[i, j] = parent2_w[i, j]
        w.append(child_w)
